#20241219.try9换用nash单位线，basicmodel
import math
...
class HydrologicalModel(pl.LightningModule):
    ...
    def forward(self, x, initial_values):
        ...
        uh[0:] = 1 / (K * gamma_N) * (t / K) ** (N - 1) * torch.exp(-t / K)  # 矢量化的公式
        #主循环迭代
...
# 示例参数，可以根据具体需求进行调整
canshu = np.zeros((8, 4))
canshu[0,0:4]=[0,50,6,14]#D
canshu[1,0:4]=[0,5,6,0]#EP
canshu[2,0:4]=[90,250,6,125]#WM
canshu[3,0:4]=[0.1,0.4,6,0.4]#B
canshu[4,0:4]=[0,10,6,8.9]#FC
canshu[5,0:4]=[0.01,0.99,6,0.91]#CG
canshu[6,0:4]=[0.01,10,6,10]#N
canshu[7,0:4]=[0.01,10,6,0.177]#K
...
torch.save(model.state_dict(), 'try10.pth')
