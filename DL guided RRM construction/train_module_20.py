...
class HydrologicalModel(pl.LightningModule):
    def __init__(self, s1):
        ...
        self.gru5 = nn.GRU(s1[2][0], s1[2][1], batch_first=True)
    ...
    def forward(self, x, initial_values):
        ...
        for t in range(x.size(1)):
            ...
            in1 = p
            temp = self.fc2(torch.relu(self.fc1(in1)))  # (batch_size, output_size)
            out1 = temp[:, 0].unsqueeze(1)
            outputs1 = torch.cat((outputs,out1), dim=1)
            in2 = torch.cat((out1, state1), dim=1)
            temp = self.fc4(torch.relu(self.fc3(in2)))  # (batch_size, output_size)
            out2 = temp[:, 0].unsqueeze(1)
            state1 = temp[:, 1].unsqueeze(1)
            outputs = torch.cat((outputs,out2), dim=1)
        out3 = self.fc6(torch.relu(self.gru5(outputs1.unsqueeze(-1))[0])).squeeze(-1)
        return outputs+out3
