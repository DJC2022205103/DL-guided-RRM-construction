...
class HydrologicalModel(pl.LightningModule):
    def __init__(self, s1):
        ...
        self.fc7 = nn.Linear(s1[3][0], s1[3][1])
        self.fc8 = nn.Linear(s1[3][1], s1[3][2])
        self.fc9 = nn.Linear(s1[4][0], s1[4][1])
        self.fc10 = nn.Linear(s1[4][1], s1[4][2])
        self.gru11 = nn.GRU(s1[5][0], s1[5][1], batch_first=True)
        self.fc12 = nn.Linear(s1[5][1], s1[5][2])
        self.criterion = nn.MSELoss()

    def forward(self, x, initial_values):
        outputs1 = torch.tensor([], requires_grad=True)
        outputs2 = torch.tensor([], requires_grad=True)
        ...
        for t in range(x.size(1)):
            ...
            in4 = torch.cat((out3,state2), dim=1)
            temp4 = self.fc8(torch.relu(self.fc7(in4)))  # (batch_size, output_size)
            out4 = temp4[:, 0].unsqueeze(1)
            state2 = temp4[:, 1].unsqueeze(1)
            in5 = out2
            temp5 = self.fc10(torch.relu(self.fc9(in5)))  # (batch_size, output_size)
            out5 = temp5[:, 0].unsqueeze(1)
            outputs1 = torch.cat((outputs1,out4), dim=1)
            outputs2 = torch.cat((outputs2,out5), dim=1)
        out6 = self.fc12(torch.relu(self.gru11(outputs2.unsqueeze(-1))[0])).squeeze(-1)
        return outputs1+out6
...
model = HydrologicalModel(
    s1=[[1,3,1],[2,3,2],[1,3,1],[2,3,2],[1,3,1],[1,2,1]]
)
