...
class HydrologicalModel(pl.LightningModule):
    ...
    def forward(self, x, initial_values):
        ...
        for t in range(x.size(1)):
            if t < input_size:
                input = torch.cat((torch.zeros(x.shape[0], input_size - t, device=x.device, dtype=x.dtype), x[:, 0:t, :].squeeze(-1)), dim=1)
            else:
                input = x[:, t - input_size + 1:t + 1, :].squeeze(-1)  # 确保 input 是 (batch_size, input_size)
            temp = self.fc1(input)  # (batch_size, hidden_size)
            temp = torch.relu(temp)  # 使用 ReLU 激活
            temp = self.fc2(temp)  # (batch_size, output_size)
            xnew[:, t, :] = temp  # 维度变换，确保与 xnew 形状一致
        #主循环迭代
        for t in range(x.size(1)):
            p = xnew[:, t, :]
            pe, sp0 = self.evapotranspiration(p, sp0)
            r, W = self.runoffgeneration(pe, W)
...
# torch.save(model.state_dict(), 'try11.pth')
