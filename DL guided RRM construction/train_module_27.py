...
class HydrologicalModel(pl.LightningModule):
    ...
    def forward(self, x, initial_values):
        ...
        out3 = self.fc6(torch.relu(self.gru5(x)[0])).squeeze(-1)
        state1 = torch.zeros_like(x[:, 0, :])
        for t in range(x.size(1)):
            p = x[:, t, :]
            # input = torch.cat((pe, W), dim=1)
            in1 = torch.cat((p,state1), dim=1)
            temp = self.fc4(torch.relu(self.fc3(in1)))  # (batch_size, output_size)
            out1 = temp[:, 0].unsqueeze(1)
            state1 = temp[:, 1].unsqueeze(1)
            in2 = out1 + out3[:, t].unsqueeze(-1)
            temp = self.fc2(torch.relu(self.fc1(in2)))  # (batch_size, output_size)
