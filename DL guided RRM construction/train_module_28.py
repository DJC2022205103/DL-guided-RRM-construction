...
class HydrologicalModel(pl.LightningModule):
    ...
    def forward(self, x, initial_values):
        ...
        out1 = self.fc4(torch.relu(self.gru3(x)[0])).squeeze(-1)
        state1 = torch.zeros_like(x[:, 0, :])
        for t in range(x.size(1)):
            p = out1[:,t].unsqueeze(-1)
            in1 = p
            temp = self.fc2(torch.relu(self.fc1(in1)))  # (batch_size, output_size)
            out2 = temp[:, 0].unsqueeze(1)
            in2 = torch.cat((p, state1), dim=1)
            temp = self.fc6(torch.relu(self.fc5(in2)))  # (batch_size, output_size)
            out3 = temp[:, 0].unsqueeze(1)
            state1 = temp[:, 0].unsqueeze(1)
            outputs = torch.cat((outputs,out2+out3), dim=1)
        return outputs
