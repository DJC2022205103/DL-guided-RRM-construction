...
class HydrologicalModel(pl.LightningModule):
    def __init__(self, input_size, hidden_size, output_size, hidden_size2, theta, theta_min, theta_max):
        super(HydrologicalModel, self).__init__()
        self.fc1 = nn.Linear(input_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size, output_size)
        self.gru = nn.GRU(input_size=2, hidden_size=hidden_size2, batch_first=True)
        self.fc3 = nn.Linear(hidden_size2, 1)  # 用于将 GRU 的输出转换为最终的流量预测
    ...
    def forward(self, x, initial_values):
        ...
        for t in range(x.size(1)):
            ...
            temp = self.fc2(torch.relu(self.fc1(input)))
            r = temp[:, 0].unsqueeze(1)
            W = temp[:, 1].unsqueeze(1)
            rs, rg = self.sourcesdividing(pe, r)
            rs_doc = torch.cat((rs_doc, rs), dim=1)
            rg_doc = torch.cat((rg_doc, rg), dim=1)

        # 汇流模块使用 GRU
        combined_sources = torch.cat((rs_doc.unsqueeze(-1), rg_doc.unsqueeze(-1)), dim=-1)  # 拼接作为 GRU 输入
        gru_output, _ = self.gru(combined_sources)
        qsim = self.fc3(gru_output).squeeze(-1)  # GRU 输出经过全连接层得到最终预测
...
torch.save(model.state_dict(), 'try19.pth')
